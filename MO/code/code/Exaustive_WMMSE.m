...
for Ks_index = 1:length(Ks_matrix)
    ...
    for SNR_index = 1:length(SNR_matrix)
        ...
        for loop = 1:loop_time
            ...
            for iter = 1:max_iter
                delta = qiongju(Urf,Ubb,V,Ks,R,sigma,row,K);    
                [f,U] = wmmse_mmwave(f,V,delta,H,K,row,sigma,R,T,epsilon,P);
                for ii = 1:K
                    V(:,ii) = H(:,:,ii)*f(:,ii);
                end
                A = delta*V*V'*delta;
                rate = real(log(det(eye(K)+row/K/sigma*Ubb'*Urf'*delta*V*V'*delta*Urf*Ubb/(Ubb'*Urf'*Urf*Ubb))));
%                 rate = real(log(det(eye(K)+row/K/sigma*U'*delta*V*V'*delta'*U/(U'*U))));
